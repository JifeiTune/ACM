...
* rotate(Point p,double angle) -     returns Point after rotateing the Point centering at p by angle radian w 
*/ 
struct Point{ 
  double x,y; 
  Point(){} 
...
* 创建后不断add加点，最后p.getconvex(newP);得到有效凸包newP 
*/ 
struct polygon{ 
  int n; 
  Point p[maxp]; 
...
struct circles{ 
  ...
  //SPOJVCIRCLES 求 n 个圆并的面积，需要加上 init_or() 去掉重复圆（否则 WA） 
